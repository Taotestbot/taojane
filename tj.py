...
🌠Sett/เชคค่า/Set    ➠ ดูการตั้งค่าป้องกัน
              💥 [ ระบบเตะ  ] 💥
...
🌠เปิดปิดกันรัน ➠กันรัน
🌠ลบรันทั้งหมด ➠ ล้างรัน+คิก
🌠*kicker      ➠  เรียกคิกเข้ากลุ่ม
🌠Check:on/เปิดอ่าน ➠  เปิดเช็คคนอ่าน
🌠Check:off/ปิดอ่าน ➠  ปิดเช็คคนอ่าน
🌠อ่าน      ➠  รายงานคนอ่าน
	...
	if op.type == 25: #disini :v
        ...
        #            mentionees = mention['MENTIONEES']
         ...
         #           for mention in mentionees:
          ...
          #              with open('settingan.json', 'w') as fp:
         #                   json.dump(periksa, fp, sort_keys=True, indent=4)
            elif "กันรัน:" in pesan.lower():
                xpesan = pesan.lower()
                xres = xpesan.replace("กันรัน:","")
                if xres == "ปิด":
            ...
            elif "ลบรันทั้งหมด" in pesan.lower():
                cl.sendText(msg.to,"กำลังยกเลิกเชิญ....")
                g1 = cl.getGroupIdsInvited()
                kk.sendText(msg.to,"กำลังยกเลิกเชิญ....")
                g2 = kk.getGroupIdsInvited()
                ka.sendText(msg.to,"กำลังยกเลิกเชิญ....")
                g3 = ka.getGroupIdsInvited()
                for x in g1:
                    cl.rejectGroupInvitation(x)
                pass
                cl.sendText(msg.to,"ยกเลิกสำเร็จ")
                for x in g2:
                    kk.rejectGroupInvitation(x)
                pass
                kk.sendText(msg.to,"ยกเลิกสำเร็จ")
                for x in g3:
                    ka.rejectGroupInvitation(x)
                pass
                ka.sendText(msg.to,"ยกเลิกสำเร็จ")
		   ...
		   msg.contentType = 13
            ...
            elif "ขอลิ้ง" == pesan.lower():
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = False
                    cl.updateGroup(X)
                    gurl = cl.reissueGroupTicket(msg.to)
                    cl.sendText(msg.to,"♝Ŧḗåм✯͜͡ᴛᴀø͜͜͜͡✯sᴇʟғʙᴏᴛ♝√\n http://line.me/R/ti/g/"+gurl)
                else:
                    pass
                    
            elif "ปิดลิ้ง" == pesan.lower():
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
                    X.preventJoinByTicket = True
                    cl.updateGroup(X)
                    cl.sendText(msg.to,"ทำการปิดลิ้งค์แล้ว ✓")
                else:
                    pass
            ...
            elif "เชคแบน" == pesan.lower():
                if periksa["banlist"] == {}:
                    cl.sendText(msg.to,"ไม่มีรายชื่อติดแบน")
                else:
                    mc = []
                    ...
                    cl.sendText(msg.to,"😂รายชื่อแบนริส😂\n\n😎 %s\n\nAll Banlist: %s"%(jo,str(len(cban))))
            elif "เชคดำ" == pesan.lower():
                if msg.toType == 2:
       ...
       #                 nban.append(cban[x].displayName)
        ...
        #            jo = "\n✧ ".join(str(i) for i in nban)
         #           cl.sendText(msg.to,"✧ Mimic Target List: ✧\n\n✧ %s\n\nTotal: %s"%(jo,str(len(cban))))
                ...
                cl.sendText(msg.to,"ล้างดำเรียบร้อย")
              ...
              if msg.toType == 2:
                ...
                cl.sendMessage(msg)
#---------------------------------------------------------					
            elif "ออน" in pesan.lower():
                elpsd = datetime.now() - strt
                cl.sendText(msg.to,'เวลาทำงานของบอท : %s'%(elpsd))
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                pass                    
#=========อ่านแทค================
            elif msg.text in ["é€£çµ¡å…ˆ:ã‚ªãƒ³","K on","เปิดคท","Contact:on"]:
                if wait["contact"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"เปิดอ่านคอนแทค")
                    else:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["é€£çµ¡å…ˆ:ã‚ªãƒ•","K off","ปิดคท","Contact:off"]:
                if wait["contact"] == False:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"ปิดอ่านคอนแทค")
                    else:
